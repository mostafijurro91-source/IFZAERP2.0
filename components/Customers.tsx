        ...
        <div className="max-w-7xl mx-auto space-y-4">
           <div className="flex flex-col md:flex-row gap-4 items-center animate-reveal">
              <div className="flex-1 flex gap-2 items-center bg-slate-100 p-1.5 rounded-[1.8rem] shadow-inner border border-slate-200 w-full focus-within:ring-2 ring-blue-500/20 transition-all">
                 ...
                 <input autoFocus type="text" placeholder="দোকান সার্চ..." className="flex-1 p-3 bg-transparent border-none text-[13px] font-bold uppercase outline-none text-black" value={search} onChange={e => setSearch(e.target.value)} />
              ...
              <div className="flex gap-2 shrink-0">
                 ...
                 {isAdmin && (
                   <>
                     ...
                     <button onClick={() => fileInputRef.current?.click()} className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold uppercase text-[10px] shadow-lg active:scale-95 transition-all hover:bg-blue-700">📤 Import</button>
                   </>
                 )}
             ...
             return (
               <div key={c.id} className={isCompact ? "grid grid-cols-12 p-5 border-b hover:bg-blue-50/50 transition-all items-center animate-reveal" : "bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group animate-reveal"}>
                  <div className={isCompact ? "col-span-5 md:col-span-4 min-w-0 pr-4" : "flex justify-between items-start mb-4"}>
                     {isCompact ? (
                       <>
                         <p className="font-bold text-[12px] uppercase italic text-slate-800 leading-none truncate mb-1">{c.name}</p>
                  ...
                  <div className={isCompact ? `col-span-4 md:col-span-2 text-right font-bold italic text-base ${balance > 1 ? 'text-red-600' : 'text-emerald-600'}` : "mt-6 pt-6 border-t flex justify-between items-end"}>
                     {isCompact ? `${balance.toLocaleString()}৳` : (
                       <>
                          <div><p className="text-[8px] font-bold text-slate-300 uppercase mb-1">Market Due</p><p className={`text-xl font-bold italic ${balance > 1 ? 'text-red-600' : 'text-emerald-600'}`}>{balance.toLocaleString()}৳</p></div>
                          <p className="text-[8px] font-bold text-slate-300 italic uppercase">📍 {c.address}</p>
                       </>
                     )}
                  </div>
                  <div className={isCompact ? "col-span-3 md:col-span-3 flex justify-end gap-1.5" : "mt-6 flex gap-2"}>
                     ...
                     {isAdmin && <button onClick={() => handleDeleteCustomer(c.id)} className="w-9 h-9 bg-red-100 text-red-600 rounded-xl flex items-center justify-center text-xs hover:bg-red-600 hover:text-white transition-all shadow-sm active:scale-90" title="Delete">🗑️</button>}
                  </div>
               </div>
             );
           })}
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[2000] flex items-center justify-center p-4">
           <div className="bg-white p-10 md:p-14 rounded-[4rem] w-full max-w-lg shadow-2xl animate-reveal text-slate-900 overflow-y-auto max-h-[95vh] custom-scroll">
              <div className="flex justify-between items-center mb-8 border-b pb-4">
                 ...
                 <button onClick={() => setShowModal(false)} className="text-3xl text-slate-300 font-bold hover:text-red-500 transition-colors">✕</button>
              </div>
              <form onSubmit={handleSave} className="space-y-4">
                 <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">দোকানের নাম</label>
                    <input required className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold uppercase italic text-sm focus:border-blue-500 focus:bg-white transition-all text-black" placeholder="দোকানের নাম" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                 </div>
                 <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">মালিকের নাম</label>
                    <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold uppercase italic text-sm focus:border-blue-500 focus:bg-white transition-all text-black" placeholder="মালিকের নাম" value={formData.proprietor_name} onChange={e => setFormData({...formData, proprietor_name: e.target.value})} />
                 </div>
                 <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">মোবাইল নম্বর</label>
                    <input required className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold italic text-sm focus:border-blue-500 focus:bg-white transition-all text-black" placeholder="মোবাইল নম্বর" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                 </div>
                 <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">এরিয়া/ঠিকানা</label>
                    <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold uppercase italic text-sm focus:border-blue-500 focus:bg-white transition-all text-black" placeholder="এরিয়া/ঠিকানা" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                 </div>
                 
                 <div className="pt-4 border-t space-y-4">
                    <div className="space-y-1">
                       <label className="text-[9px] font-bold text-red-400 uppercase ml-4 italic">পূর্বের বকেয়া (টাকা)</label>
                       <input disabled={!!editingCustomer} type="number" className="w-full p-4 bg-red-50 border-2 border-red-100 rounded-2xl outline-none font-bold text-red-600 text-sm focus:border-red-500 focus:bg-white transition-all disabled:opacity-50" placeholder="Opening Balance" value={formData.money_amount} onChange={e => setFormData({...formData, money_amount: e.target.value})} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                       <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">ইউজার আইডি</label>
                          <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold italic text-sm focus:border-blue-500 transition-all text-black" placeholder="Portal ID" value={formData.portal_username} onChange={e => setFormData({...formData, portal_username: e.target.value})} />
                       </div>
                       <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 uppercase ml-4 italic">সিক্রেট কি (পাসওয়ার্ড)</label>
                          <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-bold italic text-sm focus:border-blue-500 transition-all text-black" placeholder="Security Key" value={formData.portal_password} onChange={e => setFormData({...formData, portal_password: e.target.value})} />
                       </div>
                    </div>
                 </div>

                 <button disabled={isSaving} type="submit" className="w-full bg-blue-600 text-white py-6 rounded-3xl font-bold uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all mt-6 hover:bg-blue-700">
